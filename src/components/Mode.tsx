...
function Mode({ img, quiz, hide }: props) {
  const [mode_, setmode] = useState(true);

  function modeTogller() {
    const mode = document.getElementById("mode");
    localStorage.setItem("mode", JSON.stringify(mode_));
    setmode(!mode_);
    const mode_check = localStorage.getItem("mode");
    if (mode) {
      if (mode_check === "true") {
